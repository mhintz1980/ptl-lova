...
export function DashboardEngine() {
  ...
  const [favoriteChartIds, setFavoriteChartIds] = useState<ChartId[]>(() => loadFavs());
  ...
  return (
    <div>
      <header>
        <button onClick={() => setTopicIndex((i) => (i + 1) % TOPIC_CONFIGS.length)}>
          Next Topic
        </button>
      </header>
      <div>
        {chartIdsToRender.map((id) => {
          ...
          return (
            <div key={id}>
              <h2>{cfg.title}</h2>
              <ChartComponent pumps={pumps} filters={filters} onDrilldown={handleDrilldown} />
            </div>
          );
        })}
      </div>
...
describe('dashboard favorites', () => {
  it('persists favorites to localStorage', () => {
    ...
    expect(afterToggle).toEqual(expect.arrayContaining(['wipByStage', 'lateOrders']));
...
it('clears a filter segment when clicked', () => {
  ...
  render(
    <FilterBreadcrumb
      filters={{ dateRange: { from: null, to: null }, stage: 'FABRICATION', customerId: 'sunbelt' }}
      onClear={handle}
    />
  );
...
export function FilterBreadcrumb({ filters, onClear }: {
  filters: DashboardFilters;
  ...
  return (
    <div className="flex flex-wrap gap-2 text-xs">
      {chips.map(([key, value]) => (
        <button key={key} onClick={() => onClear(key as keyof DashboardFilters)}>
          {key}: {String(value)}
        </button>
      ))}
    </div>
