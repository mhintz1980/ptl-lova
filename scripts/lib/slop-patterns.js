...
export const slopPatterns = {
  ...
  placeholder_text: {
    pattern:
      /(lorem ipsum|test test test|asdf|foo bar baz|placeholder|replace this|todo: implement)/i,
  ...
  disabled_linter: {
    pattern:
      /(eslint-disable|pylint: disable|#\s*noqa|@SuppressWarnings|#\[allow\()/,
    exclude: [],
    severity: 'medium',
    autoFix: 'flag',
    language: null,
  ...
  hardcoded_secrets: {
    pattern:
      /(password|secret|api[_-]?key|token|credential|auth)[_-]?(key|token|secret|pass)?\s*[:=]\s*["'`](?!\$\{)(?!\{\{)(?!<[A-Z_])(?![x*#]{8,})(?![X*#]{8,})[^"'`\s]{8,}["'`]/i,
    exclude: [
      '*.test.*',
      '*.spec.*',
      '*.example.*',
   ...
   * NPM tokens
   */
  npm_token: createSecretPattern(/npm_[a-zA-Z0-9]{36}/, 'Hardcoded NPM token'),
 ...
 */
export function getPatternsForLanguageOnly(language) {
 ...
 */
export function getUniversalPatterns() {
 ...
 * @returns {Object} Filtered patterns
 */
export function getPatternsByAutoFix(autoFix) {
 ...
 */
export function getPatternsByCriteria(criteria = {}) {
  ...
  const langPatterns = criteria.language
    ? getPatternsForLanguage(criteria.language)
    : null
  const severityPatterns = criteria.severity
    ? getPatternsBySeverity(criteria.severity)
    : null
  const autoFixPatterns = criteria.autoFix
    ? getPatternsByAutoFix(criteria.autoFix)
    : null
 ...
 */
export function getAvailableSeverities() {
 ...
 */
export function isFileExcluded(filePath, excludePatterns) {
