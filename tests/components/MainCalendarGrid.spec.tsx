...
import { getModelLeadTimes as catalogLeadTimes } from '../../src/lib/seed'
...
describe('MainCalendarGrid', () => {
  ...
  it('renders segmented events for scheduled pumps', () => {
    ...
    const segments = screen.getAllByTestId('calendar-segment')
    expect(segments.length).toBeGreaterThan(1)

    const stages = segments.map((node) => node.getAttribute('data-stage'))
