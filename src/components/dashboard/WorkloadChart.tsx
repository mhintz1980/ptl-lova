...
interface WorkloadChartProps {
  ...
  headless?: boolean
  onDrilldown?: (
    update: Partial<import('./dashboardConfig').DashboardFilters>
  ) => void
}

const buildCounts = (pumps: Pump[], type: 'customer' | 'model') => {
  const counts = pumps.reduce(
    (acc, pump) => {
...
export const WorkloadByCustomerChart: React.FC<ChartProps> = ({
  filters,
  onDrilldown,
}) => {
...
export const WorkloadByModelChart: React.FC<ChartProps> = ({
  filters,
  onDrilldown,
}) => {
  const pumps = useApp((state) => state.pumps)
