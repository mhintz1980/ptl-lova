...
export function DashboardSkeleton() {
  return (
    <div className="flex flex-col gap-6 p-2 md:p-4 min-h-[calc(100vh-80px)]">
      {/* Header Skeleton */}
      ...
      <div className="grid gap-6 grid-cols-12 grid-flow-dense auto-rows-min">
          ...
          {[1, 2, 3].map((i) => (
              <div key={`row1-${i}`} className="col-span-12 md:col-span-4 h-[450px]">
                ...
                <Card className="h-full w-full rounded-3xl border border-border/40 bg-card/50 p-5 flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                     ...
                     <div className="flex-1 w-full flex items-end justify-between gap-2 px-4">
                         {[...Array(10)].map((_, i) => (
                             <Skeleton key={i} className={`w-full rounded-t-md`} style={{ height: `${Math.random() * 80 + 10}%` }} />
                         ))}
                     </div>
                </Card>
           </div>
