...
export function applyDashboardFilters(pumps: Pump[], filters: DashboardFilters): Pump[] {
    return pumps.filter((pump) => {
        // Date Range (based on last_update or scheduledEnd? Plan didn't specify, assuming last_update or relevant date)
        // Let's use last_update for now as it's generic, or maybe scheduledEnd if we are looking at schedule.
